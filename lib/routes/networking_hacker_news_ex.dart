...
class RestApiHackerNewsExample extends StatefulWidget {
  ...
  @override
  _RestApiHackerNewsExampleState createState() => _RestApiHackerNewsExampleState();
}
...
class _RestApiHackerNewsExampleState extends State<RestApiHackerNewsExample> {
  ...
  Future<void> _getLatestArticleIds() async {
    final url = Uri.parse('https://hacker-news.firebaseio.com/v0/newstories.json');
    final response = await http.get(url);
    if (response.statusCode == 200) {
      final List<int> articleIds = List<int>.from(json.decode(response.body) as Iterable);
...
class MyHackerNewsArticle {
  ...
  MyHackerNewsArticle({this.by, this.descendants, this.id, this.score, this.time, this.title, this.type, this.url});
